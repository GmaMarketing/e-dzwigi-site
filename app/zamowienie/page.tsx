...
export default function OrderPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("https://formsubmit.co/ajax/biuro@e-dzwigi.pl", {
        method: "POST",
    ...
    <main className="bg-zinc-50 min-h-screen flex flex-col selection:bg-red-600 selection:text-white">
      ...
      <div className="flex-grow pt-32 pb-20 container mx-auto px-6">
        ...
        <div className="bg-white rounded-3xl border border-zinc-200 shadow-xl overflow-hidden">
          ...
          {/* Viewer */}
          <div ref={pdfContainerRef} className="w-full h-[800px] bg-zinc-100 relative overflow-hidden">
            {pdfSrc && (
              <iframe
                src={pdfSrc}
                className={`w-full h-full absolute inset-0 z-10 bg-white transition-opacity duration-500 ${isLoading ? "opacity-0" : "opacity-100"}`}
                title="Formularz Zamówienia PDF"
                loading="lazy"
                onLoad={() => setIsLoading(false)}
              />
            )}
            <div className={`absolute inset-0 flex flex-col items-center justify-center z-0 transition-opacity duration-300 ${isLoading ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
              {pdfSrc ? (
                <>

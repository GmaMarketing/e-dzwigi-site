  ...
  return (
    ...
    >
      <div className="container mx-auto px-6">
        <div
          className={`relative flex items-center justify-between px-6 md:px-8 py-4 border transition-all duration-500 ${
            isSolid
              ? "bg-white/90 shadow-xl rounded-2xl border-white/20"
              : "bg-transparent border-transparent"
          }`}
          style={{
            backdropFilter: isSolid ? "blur(12px)" : "blur(0px)",
            WebkitBackdropFilter: isSolid ? "blur(12px)" : "blur(0px)",
          }}
        >
            ...
            {/* Desktop Navigation */}
            <nav className={`hidden lg:flex items-center gap-0.5 xl:gap-1 transition-all duration-300 whitespace-nowrap ${
                isSolid ? "bg-zinc-100/50 p-1 rounded-full" : ""
            }`}>
                {navLinks.map((item) => (
                    <Link
                        key={item.name}
                        href={item.href}
                        className={`px-3 xl:px-5 py-2 text-[10px] xl:text-xs font-bold uppercase tracking-wider xl:tracking-widest rounded-full transition-all duration-300 ${
                            isSolid
            ...
            <div className="hidden lg:flex items-center gap-6">
                <a 
                    ...
                    className={`hidden xl:flex flex-col items-end group transition-colors duration-300 ${
                        isSolid ? "text-zinc-900" : "text-white"
                    }`}
                >
                    <span className="text-[9px] uppercase font-bold tracking-widest opacity-60">Dyspozytor 24h</span>
                    <span className="font-heading font-black text-lg group-hover:text-red-600 transition-colors tracking-tight">
                        508 313 906
                    </span>
                </a>
